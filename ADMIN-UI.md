...
- Or, if the admin selects a different tournament from the dropdown list—or "**(create
  new)**", as previously presented—, the current tournament will be implicitly paused,
  which will be indicated by an informational message at the bottom of the page.  The
  newly selected (or created) tournament will then be marked as active.

Clicking "**\[cancel\]**" or the "**admin**" label in the upper righthand corner of the
screen, will effect a return to the current stage for the active tournament.
...
applied to names.  "Person Name" (the app's representation of a full name) is last name
qualified by either first name or nick name (if specified).  For convenience, a shorter
"*Player* Name" (which is either nick name or last name) is the common identifier for the
player throughout the application.  It is used as the name of the player in the seeding
round bracket, and as part of the player's team name for subsequent brackets.
...
- This view can be used to track and/or facilitate the player registration process, which
  ...
  Player Name.
- Players may enter their ping pong ball numbers (if we are still using those) or their
  desired nick names as part of the Registration stage in the mobile app.
- If a nick name is cleared out (either here, or in the mobile app), the Player Name will
  default to the player's last name.
...
### General Note on Application Data/Navigation

All of the application screens, starting from the Players view through the remainder of
the tournament flow, will have a common paradigm for refreshing data and navigating
through the application, as indicated by the annotation here:
...
- Clicking on the **tournament name** in the upper lefthand corner (highlighted in <span
  style="color: red; font-weight: bold;">red</span>) will always navigate back to the view
  representing the **current stage** of the tournament.
- Clicking on the **view name** above the data grid (highlighted in <span style="color:
  darkorange; font-weight: bold;">orange</span>) will **refresh the grid data** in place
...
- Scores for games may be entered directly in the data grid here, otherwise scores are
  assumed to be entered by players through the mobile app as they are completed.
- The controls in the grid header may be used to search, sort, or filter by any of the
  columns in the grid.
- The links below the grid may be used to display the charts and dashboards for the
  seeding round (which will be opened in separate browser tabs).
  ...
  - More detailed descriptions of the charts and dashboards will be presented separately,
    ...
    track tournament progress and results, so they should be very familiar and generally
    self-explanatory.

For testing and simulation purposes, the "**Generate Fake Results**" button may be used to
generate random results for all of the remaining games for the Seeding Round (and will not
overwrite any results already entered).
...
want to navigate back to the Players view (using the navigation menu at the top of the
...
Here is the initial look of the Partners view, after the seeding round results have been
tabulated:
...
- As with the Seeding view, the data in the grid must be refreshed manually (by clicking
  on "Partners" above the grid) to pick up updates from mobile devices.
- The "Live Dashboard" (link below the grid) may be used to show (e.g. projected, or
  displayed on a larger screen) an autoupdating view of picks as they happen (entered
  through either admin or mobile interface).
...
well as the seed positions within each division.

The "**Create Round Robin Brackets**" button below the grid will map the division
assignments and seed numbers to pre-computed bracket matchups for the main tournament
round robin play.
...
The top two teams in each division, after tie-breaking rules have been applied, will move
onto the playoffs.  The app views (and charts, dashboards, and reports) used to track the
scores and results for the playoff rounds will work similarly to the current views for
seeding and round robin play.  Final tournament results will be computed as the final step
of the process.
