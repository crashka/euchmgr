...
# state values are chronologically sequenced
TournState = IntEnum('TournState',
                     'TOURN_CREATE '
                     'ROSTER_UPLOAD '
                     'PLAYER_NUMS '
                     'SEED_BRACKET '
                     ...
                     'TOURN_TABULATE '
                     'TEAM_RANKS')
# GEEZ, states shouldn't have starts and completions (see TournInfo, below), they should
# just be (or not be).  Let's FIX the nomenclature of this at some point!!!

# NOTE that we are being a little particular/pedantic about wording here:
...
STATE_MSGS = ["New",
              "Roster completed",
              "Player nums assigned",
              "Seeding bracket created",
              "Seeding results completed",
              "Seeding results tabulated",
              "Seeding rankings computed",
              "Partner picks completed",
              "Round robin teams created",
              "Round robin team seeds computed",
              "Round robin brackets created",
              "Round robin results completed",
              "Round robin results tabulated",
              "Team rankings computed"]

assert len(STATE_MSGS) == len(TournState)  # not ensured by zip
TournStatus = dict(zip(TournState, STATE_MSGS))
...
class TournInfo(BaseModel):
    ...
    state_start    = IntegerField()
    state_compl    = IntegerField()
    status         = TextField()
    ...
    @classmethod
    def mark_state_start(cls, state: TournState) -> None:
        """Mark the start of the specified state (and save) for the cached singleton
        """
        tourn = TournInfo.get()
        tourn.start_state(state)

    @classmethod
    def mark_state_complete(cls, state: TournState) -> None:
        """Mark the completion of the specified state (and save) for the cached singleton
        """
        tourn = TournInfo.get()
        tourn.complete_state(state)

    def save(self, *args, **kwargs):
        """Manage state/status changes
        """
        if 'state_compl' in self._dirty:
            if self.state_start != self.state_compl:
                assert not self.state_start or self.state_start < self.state_compl
                # set state_start implicitly (e.g. for for short operations)
                self.state_start = self.state_compl
            self.status = TournStatus[self.state_compl]
            # TODO: should also log this information!!!
        return super().save(*args, **kwargs)

    def start_state(self, state: TournState, auto_save: bool = True) -> None:
        """Mark the start of the specified state (and save, by default)
        """
        self.state_start = state
        if auto_save:
            self.save()

    def complete_state(self, state: TournState, auto_save: bool = True) -> None:
        """Mark the completion of the specified state (and save, by default)
        """
        self.state_compl = state
        if auto_save:
            self.save()
